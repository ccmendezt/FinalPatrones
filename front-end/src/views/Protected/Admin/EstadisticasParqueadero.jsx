...
function EstadisticasParqueadero() {
    ...
    //calcula cual es el parqueadero mas usado
    for (let i = 0; i < resultGraph.length; i++) {
        if (isNaN(resultGraph[i]) || !resultGraph[i]) {
            resultGraph[i].value = "0";
        }
        r += resultGraph[i].value+ " ";
    }
    for (let i = 0; i < resultGraph.length; i++) {
        if (max < resultGraph[i]) {
            max = resultGraph[i];
            pos = i;
        }
    }
        ...
        <div>
            ...
            <div className='container'>
                ...
                <div className='d-flex mb-3 set-middle'>
                    ...
                    <a href='/estadisticas/parqueadero'><button type="button" className="btn btn-success" style={{ maxWidth: "250px" }}>Estadistícas parqueadero</button></a>
                </div>
                <div id='acquisitions'>
                    <h3 className='inputLabel'>Estadísticas de los parqueaderos usados</h3>
                    <div className='text-left'>
                        Se han usado <b>{resultGraph.length} </b>Parqueaderos diferentes segun las reservas.
                    </div>
                    <div className='text-left'>
                        El parqueadero mas usado es <b>{respGraph[1]}</b> segun las reservas.
                    </div>
...
export default EstadisticasParqueadero